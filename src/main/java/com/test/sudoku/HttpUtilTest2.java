...
public class HttpUtilTest2 {
  ...
  public void solveSvetoku() throws ExecutionException, InterruptedException {
    sudokuId = httpUtil
        .sendRequest("http://svetoku.xsec.sap.corp:8888/api/svetoku/puzzles/", "Post",
    ...
    List<Callable<String>> putterList = Arrays
        .asList(putter00, putter01, putter02, putter10, putter11, putter12, putter20, putter21,
            putter22);
    List<Future<String>> putterFutures = threadpool.invokeAll(putterList);
    for (Future<String> future : putterFutures) {
      future.get();
    }
    ...
    private String putBlock(int row, int col, String result) {
      ...
      try {
        String res = httpUtil.sendJsonRequest(
            "http://svetoku.xsec.sap.corp:8888/api/svetoku/puzzles/" + sudokuId + "/blocks/" + row
                + "/" + col, "Put", result);
  ...
  private static class SudokuGetter implements Callable {
    ...
    private String getBlock(String sudokuId, int row, int col, char[][] board) {
      ...
      String block = httpUtil.sendRequest(
          "http://svetoku.xsec.sap.corp:8888/api/svetoku/puzzles/" + sudokuId + "/blocks/" + row
              + "/" + col, "Get", new HashMap<>(), new HashMap<>(),
